...
- **本项目自己不刷写固件。** 它作为第二个 gf 客户端在运行时接管一个硬件层,对原厂的改动都在**内存**里 ——
  它自己的设置(每个音源的接管开关 + 语言)会落盘到 `/HBpersistence/dev/etc/studio.conf`,日志和锁也是普通文件。**断电即恢复原厂界面。**
  ⚠️ **但别用 kill/slay 停它** —— 被 kill 的进程不归还图层,最后一帧会留在屏上直到重启,
  原厂盖不回来。停它一律 `touch /tmp/studio.stop`。
- 🚩 **但"本项目不刷写"≠"你不需要刷写"。** 依赖**两段**必须先刷进固件的 code cave:
  音量/切换音源/调频走 **IFS1**;让原厂别响应 SOURCE 硬键走 **IFS2**。
  两段都**不由**姊妹项目 [porsche-pcm31-mods](https://github.com/WillCoder/porsche-pcm31-mods) 提供
  (它刷的是另外两段蓝牙补丁),而是来自未公开的研究工作区。
  **刷写会写 flash,有不可恢复的砖机风险。**
...
> **一句话:它自己不刷 flash,但切源/音量/调频依赖一次已完成的 **IFS1** 刷写、
> 挡 SOURCE 键还依赖一次 **IFS2** 刷写;而且它会动别人进程的内存 —— 别在开车的时候玩。**
