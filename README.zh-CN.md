...
> (来自姊妹项目的工具链)。没有那段 cave,这三条命令会返回错误并记日志,其余部分照常工作。
...
- **Studio 自己就是个普通进程。** 它不刷固件、不改磁盘上的任何东西。杀掉进程或者断电,原厂界面原封不动地回来。
...
那段 cave **不在这个仓库里** —— 它由姊妹项目
[porsche-pcm31-mods](https://github.com/WillCoder/porsche-pcm31-mods) 刷入。
没有它,这三条命令会返回错误并记日志,界面其余部分不受影响。

`studio/platform/puppet_addr.h` 里是那段 cave 的地址。**它们只对一台台架上的一个固件版本有效** ——
